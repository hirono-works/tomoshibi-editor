    ...
    function loadProjects() {
        ...
        if (savedProjects) {
            ...
            defaultProject.content =
                ...
                "<p><b>【基本の使い方】</b></p>" +
                "<p><b>〇始め方</b></p>" +
                "<p>・HOME画面の「＋」ボタンを押すと、新しいプロジェクトを作成して執筆を始められます。</p>" +
                "<p>・作成済みのプロジェクトを選択すると、いつでも続きから編集できます。</p>" +
                "<p><br></p>" +
                "<p><b>〇作品情報（ファイル名）</b></p>" +
                "<p>・サイドバーの「作品情報」にある「ファイル名」からいつでも自由に変更できます。</p>" +
                "<p>・ここで設定したファイル名が、各種出力時や保存時のファイル名に反映されます。</p>" +
                "<p>・外部ファイルを読み込んだ場合は、そのファイル名が自動で設定されます。</p>" +
                "<p><br></p>" +
                "<p><b>〇表示設定（文字数・行数）</b></p>" +
                "<p>・「1行の文字数」を変更すると、フォントサイズが画面に合わせて自動で最適化されます。</p>" +
                "<p>・「1ページの行数」を設定することで、現在の執筆量がサイドバー左下に「ページ数」としてリアルタイムに計算・表示されます。</p>" +
                "<p><br></p>" +
                "<p><b>〇追尾モード</b></p>" +
                "<p>・オンにすると、文字を入力・編集している位置へ自動的に画面がスクロールして焦点を合わせます。</p>" +
                "<p><br></p>" +
                "<p><b>〇ルビ（ポップアップ）と書式</b></p>" +
                "<p>・「ポップアップ」をオンにすると、エディタ上の文字を選択した際にルビ入力ポップアップが表示されます。</p>" +
                "<p>・ポップアップ内、またはショートカットキー（Ctrl+B / Ctrl+I）を使うことで、選択部分を太字や斜字に装飾することも可能です。</p>" +
                "<p><br></p>" +
                "<p><b>〇書体とテーマ</b></p>" +
                "<p>・フォントは「しっぽり明朝」「Noto Serif JP」「標準明朝」「ゴシック」から好みに合わせて切り替えられます（※作品全体に適用されます）。</p>" +
                "<p>・「テーマ」ボタンでライトモードとダークモードを切り替えられます。</p>" +
                "<p>・「全画面表示」ボタンを押すと、執筆に没頭できるフルスクリーンモードになります（Escキーで解除できます）。</p>" +
                "<p><br></p>" +
                "<p><b>〇ファイル操作と保存</b></p>" +
                "<p>・【自動保存】編集した内容はブラウザに自動で保存されます。「保存」ボタン（Ctrl+S）を押すことで、任意のタイミングで手動保存（およびローカルファイルへの上書き保存）も行えます。</p>" +
                "<p>・【読み込み】外部のテキストファイル（.txt）やWordファイル（.docx）をエディタ内に読み込んで編集できます。テキスト内のルビは青空文庫形式（|文字《ルビ》）として自動で解析されます。</p>" +
                "<p>・【Text出力】現在の執筆内容を青空文庫形式のテキストファイルとして手元にダウンロードします。最も確実なバックアップ方法です。</p>" +
                "<p><br></p>" +
                "<p><b>〇キャラ・プロット管理</b></p>" +
                "<p>・サイドバーのタブを切り替えることで、登場人物の設定や物語のプロットをメモとして残しながら執筆を進められます。</p>" +
                "<p><br></p>" +
                "<p><b>〇メニューの開閉</b></p>" +
                "<p>・サイドバー右上の「左矢印（＜）」ボタンを押すとメニューが格納され、執筆画面を広く使えます。隠れた状態のときは、左上の「三本線（メニュー）」ボタンからいつでも再表示できます。</p>" +
                "<p><br></p>" +
                "<p><b>【重要な注意点】</b></p>" +
                "<p>・【データ消失のリスク】本エディタのプロジェクトは、デフォルトではブラウザの保存領域（ローカルストレージ）にのみ保存されています。そのため、<b>「ブラウザのキャッシュクリア」「シークレットモードでの利用」「ブラウザやOSのアップデート・不具合」</b>などにより、データが完全に消去されてしまう可能性があります。</p>" +
                "<p>・【定期的なバックアップを】万が一のデータ消失を防ぐため、こまめに「Text出力」を行って外部へバックアップを保存するか、HOME画面にある「一括出力」機能を利用して定期的に全データを書き出すようにしてください。</p>" +
                "<p>・【開発中機能について】「Word出力」や「Google Driveクラウド連携」などの一部機能は現在開発中（無効化）です。今後のアップデートをお待ちください。</p>";
